...
使用`Process`插件创建交互器时，请确保Python将需要输出的`字符串`编码并解码为`GBK`，同时去除错误字符，最后将`group.json`内的编码改为`GBK`。
...
另外，使用`__init__.py`作为文件名时，**请您看好喽这是两边各两根下划线(`_`)，不要Pull Requests被Close了还不知道自己文件名写错了，谢谢合作，开发者没有帮您改文件名的义务。**
## 指南
